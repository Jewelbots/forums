...
export default Ember.Controller.extend(ModalFunctionality, {
  ...
  hideSignup() {
    ...
    this.set('accountBirthday', $.cookie('user_birthday'));
  ...
  stripeReturnCheck() {
    if ($(".stripe-button-el").prop('disabled') == true) {
        ...
        if($('.stripe_checkout_app').css('display') == 'block') {
            Ember.run.later(this, function () {
                this.stripeReturnCheck();
            }, 1000);
        }
    }
  },
  ...
  actions: {
    ...
    checkBirthday() {
      // if(!this.nextDisabled)
      {
        ...
        var str_birthday = birthday_year + '-' + birthday_month + '-' + birthday_date;

        var pattern =/^([0-9]{4})\-([0-9]{2})\-([0-9]{2})$/;

        if(this.get('accountBirthday')) $.cookie('user_birthday', str_birthday);

        $('#error_birthday_msg').css('display', 'none');

        if(!this.get('accountBirthday') || !pattern.test(str_birthday)) {
          $('#error_birthday_msg').css('display', 'block');
          return false;
        }

        var current_year  = new Date().getFullYear();
        var age = current_year - birthday_year;
